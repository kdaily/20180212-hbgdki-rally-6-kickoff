...
myProject <- Project(name="My new fancy project")
...
Every file gets a unique identifier, e.g. [syn11689055](https://www.synapse.org/#!Synapse:syn11689055/) and DOI: https://doi.org/10.7303/syn11689055 (most current version)
...
> Update files without removing or overwriting them (e.g. [syn3158499.18](https://www.synapse.org/#!Synapse:syn3158499.18))
...
# Version 1
f <- synStore(File("/tmp/foo.csv", parent="syn11801621"))
...
# Query the file view
results <- synTableQuery(q)$asDataFrame()

results %>% select(-ROW_ID, -ROW_VERSION) %>% head %>% knitr::kable()
```
